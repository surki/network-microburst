...
network-microburst.bpf.per_cpu_legacy.o: network-microburst.bpf.c build/libbpf/libbpf.a
	$(CLANG) -mcpu=v3 -g -O2 -Wall -Werror -D__USER_SPACE_PERCPU_COMPUTE_ONLY -D__TARGET_ARCH_$(ARCH) -include $(PWD)/legacy.h -I$(PWD)/build/libbpf $(CFLAGS) -I./include/$(ARCH) -c -target bpf $< -o $@
