...
RUN dpkg --add-architecture arm64 && \
    ...
    apt-get update && \
    apt-get install --no-install-recommends -y clang-15 libelf-dev libelf-dev:${FOREIGN_ARCH_PACKAGE_SUFFIX} \
        libbpf-dev libbpf-dev:${FOREIGN_ARCH_PACKAGE_SUFFIX} gcc gcc-${FOREIGN_ARCH}-linux-gnu binutils binutils-${FOREIGN_ARCH}-linux-gnu
