...
RUN dpkg --add-architecture arm64 && \
    ...
    FOREIGN_ARCH=$(case $(dpkg --print-architecture) in "amd64") echo "arm64";; "arm64") echo "amd64";; *) echo "unknown arch $CURR_ARCH"; exit 1;; esac;) && \
    apt-get update && \
    apt-get install --no-install-recommends -y libelf-dev gcc clang-15 && \
    apt-get install --no-install-recommends -y libelf-dev:${FOREIGN_ARCH} gcc:${FOREIGN_ARCH}
